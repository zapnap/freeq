...
export class FreeqClient extends EventEmitter {
    ...
    get joinedChannels() { return this._joinedChannels; }
    /** Active AV sessions. */
    get avSessions() { return this._avSessions; }
    /** Active AV session ID we're participating in. */
    get activeAvSession() { return this._activeAvSession; }
    /** Server origin for API calls. */
    get serverOrigin() {
        if (this.opts.serverOrigin)
            return this.opts.serverOrigin;
        try {
    ...
    connect() {
        ...
        const serializedHandleLine = (line) => {
            lineQueue = lineQueue.then(() => this.handleLine(line)).catch((e) => console.error('[freeq-sdk] line handler error:', e));
        };
    ...
    reconnect() {
        if (!this.opts.url || !this.opts.nick)
            return;
        this.transport?.disconnect();
        this.transport = null;
    ...
    sendMarkdown(target, text) {
        ...
        const tags = { '+freeq.at/mime': 'text/markdown' };
        if (isMultiline)
            tags['+freeq.at/multiline'] = '';
                ...
                const fetchWithRetry = async () => {
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            ...
                            if (!r.ok)
                                throw new Error('broker refresh failed');
                            return r.json();
                        }
                        catch (e) {
    ...
    nickToDid = null;
    resolveNickToDid(targetNick) {
        return this.nickToDid?.(targetNick);
    }
    async signedPrivmsg(target, text, extraTags) {
        const sig = await signing.signMessage(target, text);
        const tags = { ...extraTags };
        if (sig)
            tags['+freeq.at/sig'] = sig;
    ...
    cacheEchoPlaintext(ciphertext, plaintext) {
        ...
        if (this.echoPlaintextCache.size > 100) {
            ...
            for (const [k, v] of this.echoPlaintextCache) {
                if (now - v.ts > 60_000)
                    this.echoPlaintextCache.delete(k);
            }
        }
    }
        ...
        switch (msg.command) {
            ...
            case '900':
                ...
                if (this._authDid) {
                    ...
                    e2ee.initialize(this._authDid, this.serverOrigin).catch((e) => console.warn('[e2ee] Init failed:', e));
                }
                break;
            case '903':
                if (this.sasl?.did) {
            ...
            case '001': {
                ...
                if (!this.sasl?.did && toJoin.length === 0)
                    toJoin.push('#freeq');
                for (const ch of toJoin) {
                    if (ch.trim())
                        this.raw(`JOIN ${ch.trim()}`);
                }
                this.autoJoinChannels = [];
                if (this.sasl?.did)
            ...
            case 'JOIN': {
                ...
                const actorClass = (msg.tags?.['freeq.at/actor-class'] || msg.tags?.['+freeq.at/actor-class']);
            ...
            case 'NOTICE': {
                ...
                if (ticketMatch) {
                    ...
                    if (activeId)
                        this.emit('avTicket', activeId, ticketMatch[1]);
                    break;
                }
                // Pin/unpin sync
            ...
            case 'TAGMSG': {
                ...
                if (deleteOf) {
                    this.emit('messageDeleted', bufName, deleteOf);
                    break;
                }
                ...
                if (avState && avId) {
                    this.handleAvSessionState(avId, avState, target, msg.tags['+freeq.at/av-actor'] || '', parseInt(msg.tags['+freeq.at/av-participants'] || '0', 10), msg.tags['+freeq.at/av-title']);
                }
                break;
            }
            case 'TOPIC': {
            ...
            case '401': {
                ...
                this.emit('systemMessage', failNick || 'server', `${failNick} is offline — message saved, they'll see it next time they connect`);
                break;
            }
            case '404':
            ...
            case '673': {
                ...
                if (match && whoisNick) {
                    this.emit('memberJoined', '', { nick: whoisNick, actorClass: match[1] });
                }
                if (!this.backgroundWhois.has(whoisNick.toLowerCase())) {
    ...
    async handleAuthenticate(msg) {
        ...
        try {
            ...
            for (let i = 0; i < bin.length; i++)
                rawChallengeBytes[i] = bin.charCodeAt(i);
    ...
    handleAvSessionState(sessionId, action, channel, actorNick, _participantCount, title) {
        ...
        switch (action) {
            ...
            case 'ended': {
                if (existing) {
                    const ended = { ...existing, state: 'ended', participants: new Map() };
